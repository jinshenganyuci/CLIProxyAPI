...
本分支基于 CLIProxyAPI `v7.2.146`（上游提交
`d31b15916d15b550bbf388fd6da4a47d4d864109`），目标是让每个 Codex OAuth
...
在 `v7.2.146-codex-identity.1` 中，OAuth 保存后的运行时同步从最终落盘文件重新
...
2. 把 `CLI_PROXY_IMAGE` 改为
   `jinshenganyuci/cli-proxy-api:codex-identity-v7.2.146.1`，保持现有 volumes 不变。
